...
#{{{
    ...
    num_founders = 2**(int(input_strings[0][1])) #correction of +1 because we use number of observed crossovers
    num_haplotypes = 2*num_founders

    #print(len(admixed_branches),num_haplotypes) 
    ...
    founder_haplotypes = [str(x) for x in range(1, num_haplotypes+1)]
...
def ancestryParse_gco(input_strings, admixed_branches):
    '''
    Args:
    input_strings : Takes input strings - one for each chromosome - in bp format (output from ped-sim) 
    admixed_branches : list of admixture types for each branch ( list length should be equal to number of founders)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--t", "-t", required=True, type=int, help="set time since admixture")
    ...
    admixstring_list=list('10'*(2**(args.t-1)))
    ...
    outpairs=[]
    if args.gcomode:
        for i in range(len(inlines)): #Read in pairs of lines
            if i%2==0:
                inpair=[inlines[i][:-1],inlines[i+1][:-1]]
                outpairs.append(ancestryParse_gco(inpair,admixstring_list))
            else:
                continue
    else:
        for i in range(len(inlines)): #Read in pairs of lines
            if i%2==0:
                inpair=[inlines[i][:-1],inlines[i+1][:-1]]
                outpairs.append(ancestryParse(inpair,admixstring_list))
            else:
                continue

    print('\n'.join([x for y in outpairs for x in y])) #Flatten and print

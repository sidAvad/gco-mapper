...
def bk_tree_lookup3(root_node: BKTreeNode,
                   vector: DiscreteVector,
                   distance: Callable[[DiscreteVector, DiscreteVector], Tuple[int,int]],
                   avoid_query_element: Optional[Any] = None,
                   collect_all: bool = True,
                   avoid_dist_zero: bool = True) -> Tuple[List[BKTreeNode], Tuple[int,int]]:
    """
    Lookup the nearest neighbor(s) to the given vector and return their nodes.
    ...
    while node_list:
        node = node_list.pop()
        dist = distance(node.vector, vector) #dist is now a tuple that contains distance and location of edits
        # We do not return nodes that have no elements.
        ignore_this_node = (len(node.elements) == 0) or (avoid_dist_zero and dist[0] == 0)
        if dist[0] == 0:
            exact_matches.append(node)
        if avoid_query_element is not None:
            ignore_this_node = ignore_this_node or (len(node.elements) == 1 and node.elements[0] == avoid_query_element)
        if not ignore_this_node:
            if dist[0] < dist_best[0]:
                # We've got a strictly better distance, clear the old set
                results = [node]
                dist_best = dist
            elif collect_all and dist[0] == dist_best[0]:
                results.append(node)
        for next_dist, next_node in node.children.items():
            # Everything at or below next_node has dist(node.vector, ?) = next_dist.
            bound = abs(next_dist[0] - dist[0])
            if bound < dist_best[0] or (bound <= dist_best[0] and collect_all):
                node_list.append(next_node)
    return results, dist_best, exact_matches
